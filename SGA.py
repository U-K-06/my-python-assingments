...
def update_df(df_of_csv_data: Path) -> pd.DataFrame:
    ...
    try:  # handling empty csv files
        df_of_csv_data = pd.read_csv(CSV_DATA_PATH)  # reading the data
                      ...
                      print('File Not Found!\n possibly because the file has been deleted or moved')
...
def delete_entry(df_to_del_entry: pd.DataFrame, name_entry_to_del: str) -> pd.DataFrame:
    ...
    if df_to_del_entry.shape[0] == 1:
        df_to_del_entry = pd.DataFrame()
        return df_to_del_entry
...
def assign_grade(df_with_avg: pd.DataFrame) -> pd.DataFrame:
    ...
    df_with_avg = calc_avg(
        df_with_avg
    )  # if the dataframe is without an average column or some values are nan due to addition of entries after creation of average column
    grades_arr: list[str] = []
    avg_arr: list[int] = [
        round(i) if not isnan(i) else 0 for i in df_with_avg["Average"]
    ]
...
def calc_avg(df_csv_val: pd.DataFrame, mode: str = "all") -> pd.DataFrame | float:
    ...
    if mode == "all":
        ...
        for i in range(df_csv_val.shape[0]):
            if i != -1:
                if "Average" in df_csv_val.columns or "Grade" in df_csv_val:
                    vals.append(df_csv_val.iloc[i, 1:-1].mean())
                else:
                    vals.append(df_csv_val.iloc[i, 1:].mean())
...
def compare_vals(
    ...
    if mode == "all":
        df_of_data_for_comparision.plot(kind="bar", xlabel="Names", ylabel="Marks")
        plt.xticks(
...
if not CSV_DATA_PATH or not CSV_DATA_PATH.is_file():  # handle invalid inputs
    print("ERROR!")
    print("Invalid file path \n either the file does not exist or the extension(.csv) was not providedg")
    have_csv_ext = CSV_DATA_PATH.suffix == '.csv'
    if input(r'Create new file with the  name provided? (Y\N)').lower() == 'y':
                  print('Creating new file with the name provided in the current working directory')
                  Path((str(CSV_DATA_PATH)+'.csv' if not have_csv_ext  else str(CSV_DATA_PATH))).touch()
                  CSV_DATA_PATH = str(CSV_DATA_PATH) + '.csv' if not have_csv_ext else CSV_DATA_PATH
    else:
                  print('------------------------------------\n')
                  input('Enter to exit')
                  exit(0)
...
df_of_csv_data = update_df(CSV_DATA_PATH) if not df_of_csv_data else df_of_csv_data  # get the data from the dataframe
if df_of_csv_data.empty:
    print("Empty CSV file detected")
    print('_____________________')
    no_subjects = user_input_handling(
        input("Enter the number of subjects to be stored:\n"), int
    )
else:
...
while running:
    ...
    if command == "empty":
        print(
            """Are you sure you want to empty the csv file?\n
